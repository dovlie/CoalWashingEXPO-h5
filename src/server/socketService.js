export default class NodeSocketService {
  /**
   * 单例
   */
  ...
  static get Instance() {
    if (!this.instance) {
      this.instance = new NodeSocketService()
    }
  ...
  connect() {
    ...
    this.ws = new WebSocket(`${process.env.VUE_APP_BASE_URL}`)
    // 连接成功的事件
    this.ws.onopen = () => {
      console.log('连接后端服务端成功')
      this.connected = true
      // 重置重新连接的次数
    ...
    this.ws.onmessage = msg => {
      console.log('从后端获取到了数据：' + msg.data)
